...
class RoadmapGenerateRequest(BaseModel):
    goal: str  # e.g., "Full Stack Web Developer"
...
def _run_crewai_roadmap(goal: str, skills_str: str, duration: int) -> Dict:
    ...
    # We attempt importing CrewAI. If not installed or missing key, we handle cleanly.
    try:
        ...
        result = crew.kickoff()
        # Parse result... (we would parse output as JSON, since this is complex, we use structured prompt)
    except Exception:
        pass
...
"""
    ...
    try:
        ...
        # Static Fallback Builder matching requested length
        timeline = []
        tasks = []
